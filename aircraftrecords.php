<?php
    ...
    /*if (!isset($_POST['aircraft']) || (!isset($_POST['cost']) && !isset($_POST['delete']))) {
        $_SESSION['aircraft_record_err'] = "Something is horribly wrong, or you've forgotten to set a field.";
    ...
    if (isset($_POST['delete'])) {
        ...
        if (delete_record($aircraft)) {
            $_SESSION['aircraft_record_msg'] = "Deletion successful";
        } else {
            $_SESSION['aircraft_record_err'] = "Deletion unsuccessful";
        }
    }
    if (isset($_POST['changerecord'])) {
        ...
        if (update_cost($aircraft, $cost)) {
            $_SESSION['aircraft_record_msg'] = "Update successful";
        } else {
            $_SESSION['aircraft_record_err'] = "Update unsuccessful";
        }
    }
    if (isset($_POST['addrecord'])) {
        ...
        if (add_aircraft($reg_number, $cost, $tid)) {
            $_SESSION['aircraft_add_msg'] = "Addition successful";
        } else {
            $_SESSION['aircraft_add_err'] = "Addition unsuccessful";
        }
    }
    header('Location: '.SITE_ROOT.'/admin.php');
?>
